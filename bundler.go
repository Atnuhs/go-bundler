...
func Bundle(pkgs []*packages.Package) (string, error) {
	// init
	b := &Bundler{pkgs: pkgs}
	...
	// bundle
	file := b.buildDeclFile()
	b.applyPrefixes(file)

	// format
	buf := bytes.NewBuffer(make([]byte, 0, 1024))
	fmt.Fprintln(buf, `// Code generated by summarizer; DO NOT EDIT.`)
	fmt.Fprintln(buf)
	if err := format.Node(buf, b.pkgs[0].Fset, b.bundled); err != nil {
		return "", err
	}
...
func (b *Bundler) Init() error {
	if err := b.searchMainPkg(); err != nil {
		slog.Error("failed to search main package", "error", err)
	}
...
func (b *Bundler) buildDeclFile() *ast.File {
