...
// Bundler orchestrates the entire bundling process
type Bundler struct {
	fset         *token.FileSet
...
func NewBundler() *Bundler {
	...
	return &Bundler{
		fset:     fset,
		pkgCache: pkgCache,
		analyzer: NewUsageAnalyzer(fset, pkgCache),
	}
}
...
func (b *Bundler) performCallGraphAnalysis() error {
	...
	if err := b.analyzer.AnalyzeUsage(mainPkg, b.dependencies); err != nil {
		return err
	}
...
func (b *Bundler) processFile(file *ast.File, pkgPrefix, depPath string, output *strings.Builder) error {
	usedSymbols := b.analyzer.GetUsedSymbols()[depPath]
	if usedSymbols == nil {
